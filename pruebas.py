...
def agregarDispositivo():
    ...
    sistemaOperativo = (consultaSNMP(nombreComunidad,direccionIP,puerto,'1.3.6.1.2.1.1.1.0'))
    ...
    futures.append(executor.submit(actualizarRRD, nombreComunidad, direccionIP, documentorrd, dispositivo));
...
def iniciarUpdates():
    for dispositivo in datos:
        futures.append(executor.submit(actualizarRRD, dispositivo['nombreComunidad'], dispositivo['direccionIP'], dispositivo['direccionIP']+".rrd", dispositivo))

## Generación del menú interactivo
datos = leerDispositivos();
iniciarUpdates();
print("Se han inicializado los dispositivos")
while True:
    os.system("clear")
